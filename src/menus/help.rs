...
impl HelpMenu {
    pub fn new() -> Self {
        ...
        let job_overview = HelpCategory::new(
            "Job Overview", job_overview_entries);
        ...
        let job_actions = HelpCategory::new(
...
impl HelpMenu {
    pub fn open(&mut self, _selected_category: usize) {
        ...
        // set offset
        let category_lengths = self.categories.iter()
            .map(|c| c.entries.len())
            .collect::<Vec<usize>>();
        self.offset = 0;
    ...
    pub fn scroll_down(&mut self) {
        ...
        let total_entries = self.categories.iter()
            .map(|c| c.entries.len())
            .sum::<usize>();
    ...
    pub fn render(&mut self, f: &mut Frame, _area: &Rect) {
        ...
        for category in self.categories.iter() {
            ...
            for entry in category.entries.iter() {
                ...
                let desc_height: u16 = (description.line_count(desc_width) as u16)
                    .min(render_rect.height);
                let mut entry_rect = render_rect.clone();
                entry_rect.height = desc_height;
                render_rect.y += desc_height;
                render_rect.height = render_rect.height
                    .saturating_sub(desc_height);

                let layout = Layout::default()
                    .direction(Direction::Horizontal)
                    .constraints([Constraint::Min(1),
                                  Constraint::Length(desc_width),
                    ].as_ref(),).split(entry_rect);
